...
final class ContentView: UIView, UIGestureRecognizerDelegate {
    ...
    // Guard against stale async focused-input responses; token is bumped on
    ...
    func updateFocusedInputRelocation(
        ...
        session.focusedInputMetrics { [weak self] metrics in
            DispatchQueue.main.async {
                guard let self else { return }
                guard token == self.focusedInputToken else { return }
                guard session === self.session, self.isDisplaying(session: session) else { return }
                ...
                let viewportFrame = engineFrame.inset(by: UIEdgeInsets(
                    top: max(0, self.contentTopInset - self.toolbarTopOffset),
                    left: 0,
                    bottom: max(0, self.dynamicToolbarMaxHeight - self.contentTopInset + self.contentBottomOffset),
                    right: 0
                ))
