...
final class ReaderSettingsViewController: UIViewController, UIPopoverPresentationControllerDelegate {
    ...
    private let backgroundView = UIVisualEffectView(effect: UIBlurEffect(style: .appChromeMaterial))
    ...
    override func viewDidLoad() {
        ...
        configureContent()
        refreshAppearance()
        // brightnessDidChangeNotification is iOS 13+; iOS 12 just reads once.
        ...
        if #available(iOS 15.0, *), !asPopover {
            ...
            if let sheet = sheetPresentationController {
                ...
                if #available(iOS 26.0, *) {
                    ...
                    backgroundView.layer.applyContinuousCornerCurve()
    ...
    private func makeReaderContainer() -> UIVisualEffectView {
        ...
        material.contentView.backgroundColor = .appTertiarySystemFill
        return material
    }
    ...
    private func configureFontButton() {
        ...
        fontButton.setTitleColor(.appLabel, for: .normal)
        fontButton.semanticContentAttribute = .forceRightToLeft
        fontButton.tintColor = .appSecondaryLabel
        fontButton.backgroundColor = .appTertiarySystemFill
    ...
    private func makeBrightnessControl() -> UIVisualEffectView {
        brightnessSlider.minimumTrackTintColor = .appLabel
        ...
        brightnessPill.contentView.backgroundColor = .appTertiarySystemFill
    ...
    private func makeActionControls() -> UIView {
        ...
        findBackground.contentView.backgroundColor = .appTertiarySystemFill
        controlsBackground.contentView.backgroundColor = .appTertiarySystemFill
    ...
    private func makeMaterialContainer(
        ...
        let material = UIVisualEffectView(effect: UIBlurEffect(style: .appMaterial))
        material.layer.cornerRadius = cornerRadius
        material.layer.applyContinuousCornerCurve()
    ...
    private func configureOpaqueMaterialContainers() {
        materialContainers.forEach {
            $0.effect = nil
            $0.contentView.backgroundColor = .appTertiarySystemFill
        }
    }
    ...
    private func font(for type: ReaderViewFontType) -> UIFont {
        // withDesign is iOS 13+; iOS 12 gets the plain system font.
        if #available(iOS 13.0, *) {
        ...
        return .systemFont(ofSize: UX.fontSize)
    }
    ...
    private func refreshFontSelection() {
        ...
        // UIAction is iOS 13+; the class doesn't exist on iOS 12, so even
        // constructing actions must stay inside the gate (legacy menu below).
        if #available(iOS 14.0, *) {
    ...
    private func refreshFontSize() {
        ...
        fontSizeDots.arrangedSubviews.enumerated().forEach { index, dot in
            dot.backgroundColor = index + ReaderViewAppearance.minimumFontSizeStep <= step ? .appLabel : .appSystemGray4
        }
    }
