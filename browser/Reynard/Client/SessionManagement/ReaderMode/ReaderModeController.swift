...
final class ReaderModeController: AddonMessageDelegate, AddonPortDelegate {
    ...
    init(delegate: ReaderModeControllerDelegate) {
        ...
        // iOS 12 has no Concurrency runtime: plain GCD instead of Task/await.
        DispatchQueue.main.async { [weak self] in
            self?.ensureExtensionInstalled()
        }
    }
    ...
    private func ensureExtensionInstalled() {
        AddonRuntime.shared.ensureBuiltIn(
            location: Self.extensionLocation,
            id: Self.extensionID
        ) { [weak self] result in
            switch result {
            case .success(let addon):
                self?.extensionBaseURL = addon.metaData.baseURL
