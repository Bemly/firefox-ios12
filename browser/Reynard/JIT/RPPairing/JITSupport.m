...
dispatch_queue_t debugServiceQueue(void) {
    ...
    dispatch_once(&onceToken,^{
        queue = dispatch_queue_create("moe.bemly.reynard.JITSupport.DebugServiceQueue", DISPATCH_QUEUE_CONCURRENT);
    });
    return queue;
}
...
dispatch_queue_t debugSessionStateQueue(void) {
    ...
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        queue = dispatch_queue_create("moe.bemly.reynard.JITSupport.DebugSessionStateQueue", DISPATCH_QUEUE_SERIAL);
    });
    return queue;
}
...
static dispatch_queue_t endpointMonitorQueue(void) {
    ...
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        queue = dispatch_queue_create("moe.bemly.reynard.JITSupport.EndpointMonitorQueue", DISPATCH_QUEUE_SERIAL);
    });
    return queue;
}
...
static void startHeartbeat(DeviceProvider *provider) {
    dispatch_queue_t heartbeatQueue = dispatch_queue_create("moe.bemly.reynard.JITSupport.ProviderHeartbeatQueue",DISPATCH_QUEUE_SERIAL);
